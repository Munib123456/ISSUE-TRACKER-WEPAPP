...
def view_issue(issue_id):
    ...
    from app.models import get_board_by_id
    ...
    return render_template('issue.html', issue=issue, board=board, members=members, messages=messages, is_member=is_member)
...
@issues_bp.route('/issue/<int:issue_id>/resolve', methods=['POST'])
@login_required
def resolve_issue_route(issue_id):
    ...
    from app.models import get_board_by_id, update_issue_status
    board = get_board_by_id(issue['board_id'])
    
    # Only issue creator OR board creator (lecturer) can resolve
    is_authorized = (issue['created_by'] == current_user.id) or (board['created_by'] == current_user.id)
    
    if not is_authorized:
        flash('You do not have permission to resolve this issue.', 'danger')
        return redirect(url_for('issues.view_issue', issue_id=issue_id))
    
    # Check if already resolved
    if issue['status'] == 'resolved':
        flash('This issue is already resolved.', 'info')
        return redirect(url_for('issues.view_issue', issue_id=issue_id))
